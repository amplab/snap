...
void usage()
{
    fprintf(stderr, "usage: mutant-expression index inputFile {-h} {-vcf|-germline|-selectedReads DNAReadsFilename RNAReadsFilename} {-consolodatedInput inputFilename} {-outputFile outputFilename}\n");
...
int main(int argc, char* argv[])
{
    ...
    if (printHeader) {
        if (vcfStyleInput) {
            fprintf(outputFile, "Cancer_Type\tParticipantID\tDisease Abbr\tReference Assembly\tNormal DNA Analysis ID\tNormal DNA Filename\tNormal RNA Analysis ID\t"
            ...
            fprintf(outputFile, "Cancer_type\tDNA_Input_filename\tRNA_Input_Filename\tAnalysis_id\tchromosome\tpos\tID\tref\talt\tqual\tfilter\tvcfInfo\tvcfInfo2\tvcfInfo3\treference\t"
            ...
            fprintf(outputFile, "Contig\tloc\tcontig\tloc\tid\tref\talt\tqual\tfilter\tinfo\tformat\t??\tn_Matching_Reference (DNA)\tn_Matching_Variant (DNA)\tn_Matching_Neither (DNA)\tn_Matching_Both (DNA)\tn_Matching_Reference (RNA)\tn_Matching_Variant (RNA)\tn_Matching_Neither (RNA)\tn_Matching_Both (RNA)\n");
        } else {
            fprintf(outputFile, "Cancer_Type\tLocal_file_name\tHugo_Symbol\tEntrez_Gene_Id\tCenter\tNCBI_Build\tChromosome\tStart_Position\tEnd_Position\tStrand\tVariant_Classification\tVariant_Type\tReference_Allele\tTumor_Seq_Allele_1\tTumor_Seq_Allele_2\tdbSNP_RS\tdbSNP_Val_Status\t"
    ...
    while (fgets(inputBuffer, inputLineSize - 1, inputFile)) {
        ...
        for (int pass = 0; pass < ((vcfStyleInput || germlineStyleInput || selectedReadsStyleInput) ? 2 : 1); pass++) {
            ...
            if (!ReferenceGenome->getLocationOfContig(piece[Chrom], &mutationContigStartLocation, NULL, false)) {
                //
                // try prefixing it with "chr"
                //
                char chromosomeName[1000];
                sprintf(chromosomeName, "chr%s", piece[Chrom]);
                if (!ReferenceGenome->getLocationOfContig(chromosomeName, &mutationContigStartLocation, NULL, false)) {
                    //
                    ...
                    if (NULL == alternateChromName || !ReferenceGenome->getLocationOfContig(alternateChromName, &mutationContigStartLocation, NULL, false)) {
        ...
        if (vcfStyleInput) {
            fprintf(outputFile, "%s\t%s\t%d\t%d\t%d\t%d\t", cancerType, rawInputBuffer, nMatchingReference[0], nMatchingMutation[0], nMatchingNeither[0], nMatchingBoth[0]);
            if (hasNormalRNA) {
