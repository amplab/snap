	...
	Read* read1)
{
    if (!readIdsMatch(read0, read1)) {
        ...
        fprintf(stderr, "Unmatched read IDs '%s' (%d:%d) and '%s' (%d:%d).  Use the -I option to ignore this.\n", p[0], read0->getBatch().fileID, read0->getBatch().batchID, p[1], read1->getBatch().fileID, read1->getBatch().batchID);
        soft_exit(1);
    }
}
