...
class TenXAnchorAligner : public PairedEndAligner
{
public:
    TenXAnchorAligner(
        GenomeIndex  *index_,
    ...
    static size_t getBigAllocatorReservation(GenomeIndex * index, unsigned maxBigHitsToConsider, unsigned maxReadSize,
	                                         unsigned seedLen, unsigned maxSeedsFromCommandLine, 
                                             double seedCoverage, unsigned maxEditDistanceToConsider, unsigned maxExtraSearchDepth, unsigned maxCandidatePoolSize,
                                             int maxSecondaryAlignmentsPerContig);

    void *operator new(size_t size, BigAllocator *allocator) {_ASSERT(size == sizeof(TenXAnchorAligner)); return allocator->allocate(size);}
...
private:
    ...
    TenXAnchorAligner() : alignmentAdjuster(NULL) {}  // This is for the counting allocator, it doesn't build a useful object
    ...
    void allocateDynamicMemory(BigAllocator *allocator, unsigned maxReadSize, unsigned maxBigHitsToConsider, 
	                unsigned maxSeedsToUse, unsigned maxEditDistanceToConsider, unsigned maxExtraSearchDepth,
					unsigned maxCandidatePoolSize, int maxSecondaryAlignmentsPerContig);
