...
GenomeIndex::fillInLookedUpResults(GenomeLocation lookedUpLocation, _int64 *nHits, const GenomeLocation **hits, GenomeLocation *singleHitLocation)
{
	...
	void
GenomeIndex::lookupSeed(Seed seed, GenomeLocation minLocation, GenomeLocation maxLocation, _int64 *nHits, const GenomeLocation **hits,
	_int64 *nRCHits, const GenomeLocation **rcHits, GenomeLocation *singleHit, GenomeLocation *singleRCHit)
{
	lookupSeed(seed, nHits, hits, nRCHits, rcHits, singleHit, singleRCHit);

	_int64 hitCounts[NUM_DIRECTIONS] = { *nHits, *nRCHits };
	const GenomeLocation *allHits[NUM_DIRECTIONS] = { *hits, *rcHits };
	...
	for (Direction direction = FORWARD; direction < NUM_DIRECTIONS; direction++) {
		...
		_int64 low = 0;
		_int64 high = hitCounts[direction] - 1;
		while (low < high - 32) {
			_int64 mid = low + (high - low) / 2;
			if (allHits[direction][mid] <= maxLocation) {
				high = mid;
			} else {
				low = mid + 1;
			}
		...
		while (low < hitCounts[direction] && allHits[direction][low] > maxLocation) {
			low++;
		}
		_int64 end = low;
		while (end < hitCounts[direction] && allHits[direction][end] >= minLocation) {
			end++;
		}
		hitCounts[direction] = end - low;
		allHits[direction] = allHits[direction] + low;
	}

	*hits = allHits[FORWARD];
	*nHits = hitCounts[FORWARD];
	*rcHits = allHits[RC];
	*nRCHits = hitCounts[RC];
}
	
	void
GenomeIndex::lookupSeed32(Seed seed, GenomeLocation gl_minLocation, GenomeLocation gl_maxLocation, _int64 *nHits, const unsigned **hits,
		_int64 *nRCHits, const unsigned **rcHits)
{
	lookupSeed32(seed, nHits, hits, nRCHits, rcHits);
	_ASSERT(GenomeLocationAsInt64(gl_minLocation) < 0xffffffff && GenomeLocationAsInt64(gl_maxLocation) < 0xffffffff);
	unsigned minLocation = (unsigned)GenomeLocationAsInt64(gl_minLocation);
	unsigned maxLocation = (unsigned)GenomeLocationAsInt64(gl_maxLocation);

	_int64 hitCounts[NUM_DIRECTIONS] = { *nHits, *nRCHits };
	const unsigned *allHits[NUM_DIRECTIONS] = { *hits, *rcHits };
