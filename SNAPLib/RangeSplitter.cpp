...
RangeSplittingReadSupplierGenerator::generateNewReadSupplier()
{
    ...
    if (isSAM) {
        underlyingReader = SAMReader::create(DataSupplier::Default, fileName, numThreads, context, rangeStart, rangeLength);
    } else {
        underlyingReader = FASTQReader::create(DataSupplier::Default, fileName, numThreads, rangeStart, rangeLength, context);
    }
...
RangeSplittingPairedReadSupplierGenerator::generateNewPairedReadSupplier()
{
    ...
    switch (fileType) {
    case SAMFile:
         underlyingReader = SAMReader::createPairedReader(DataSupplier::Default, fileName1, numThreads, rangeStart, rangeLength, quicklyDropUnpairedReads, context);
         break;

    case FASTQFile:
         underlyingReader = PairedFASTQReader::create(DataSupplier::Default, fileName1, fileName2, numThreads, rangeStart, rangeLength, context);
         break;

    case InterleavedFASTQFile:
        underlyingReader = PairedInterleavedFASTQReader::create(DataSupplier::Default, fileName1, numThreads, rangeStart, rangeLength, context);
        break;
