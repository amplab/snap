...
FASTQReader::getNextRead(Read *readToUpdate)
{
    ...
    if (! data->getData(&buffer, &validBytes)) {
        data->nextBatch(/*false BJB - bug workaround for simple case*/);
        if (! data->getData(&buffer, &validBytes)) {
            return false;
        }
    }
    ...
    bool gzip)
{
    if (! gzip) {
        ...
        ReadReader* fastq = FASTQReader::create(DataSupplier::GzipDefault, fileName, 0, QueryFileSize(fileName), clipping);
        if (fastq == NULL) {
            delete fastq;
            return NULL;
        }
