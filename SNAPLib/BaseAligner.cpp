    ...
    if (nCandidatesForAffineGap > 0 && !skipAffineGap) {
        ...
        for (int i = 0; i < nCandidatesForAffineGap; i++) {
            ...
            if (!skipAffineGap) {
                ...
                if (candidate->score != ScoreAboveLimit) {
...
#ifdef  _DEBUG
    if (_DumpAlignments) printf("Final affine gap result score %d MAPQ %d (%e probability of best candidate, %e probability of all candidates, non ALT-aware)  at %s:%llu\n\n",
        result->score, result->mapq, scoresForAllAlignments.probabilityOfBestCandidate, scoresForAllAlignments.probabilityOfAllCandidates,
...
void BaseAligner::ScoreSet::init(SingleAlignmentResult* result) {
    ...
    bestScoreGenomeLocation = result->origLocation;
