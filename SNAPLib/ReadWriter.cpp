...
public:
    ...
    virtual bool writeRead(Read *read, AlignmentResult result, unsigned genomeLocation, bool isRC);
    ...
    bool isRC)
{
    ...
    for (int pass = 0; pass < 2; pass++) {
        ...
        if (format->writeRead(genome, &lvc, buffer, size, &used, read->getIdLength(), read, result, genomeLocation, isRC)) {
            _ASSERT(used <= size);
    ...
    PairedAlignmentResult *result)
{
    ...
    for (int pass = 0; pass < 2; pass++) {
        ...
        bool writesFit = format->writeRead(genome, &lvc, buffer, size, &sizeUsed[first],
                            idLengths[first], reads[first], result->status[first], result->location[first], result->isRC[first], true, true,
        ...
        if (writesFit) {
            writesFit = format->writeRead(genome, &lvc, buffer + sizeUsed[first], size - sizeUsed[first], &sizeUsed[second],
                idLengths[second], reads[second], result->status[second], result->location[second], result->isRC[second], true, false,
