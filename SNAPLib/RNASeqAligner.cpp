...
Authors:
    ...
    Adapted from PairedAligner.cpp, Matei Zaharia
...
struct RNASeqAlignerStats : public AlignerStats
{
    ...
    RNASeqAlignerStats(AbstractStats* i_extra = NULL);

    virtual ~RNASeqAlignerStats();
...
const int RNASeqAlignerStats::MAX_DISTANCE;
const int RNASeqAlignerStats::MAX_SCORE;

RNASeqAlignerStats::RNASeqAlignerStats(AbstractStats* i_extra)
...
RNASeqAlignerStats::~RNASeqAlignerStats()
{
    BigDealloc(distanceCounts);
    BigDealloc(scoreCounts);
}

void RNASeqAlignerStats::add(AbstractStats * i_other)
{
    AlignerStats::add(i_other);
    RNASeqAlignerStats* other = (RNASeqAlignerStats*) i_other;
...
void RNASeqAlignerStats::printHistograms(FILE* output)
{
...
RNASeqAlignerOptions::RNASeqAlignerOptions(const char* i_commandLine)
...
void RNASeqAlignerOptions::usageMessage()
{
...
bool RNASeqAlignerOptions::parse(const char** argv, int argc, int& n)
{
...
RNASeqAlignerContext::RNASeqAlignerContext(AlignerExtension* i_extension)
...
AlignerOptions* RNASeqAlignerContext::parseOptions(int i_argc, const char **i_argv, const char *i_version)
{
    argc = i_argc;
    argv = i_argv;
    version = i_version;

    RNASeqAlignerOptions* options = new RNASeqAlignerOptions(
        "snap rnaseq <genome-dir> <transcriptome-dir> <gtf-annotation> <read1.fq> <read2.fq> [-o output.sam] [<options>]\n"
    ...
    options->inputFilename = argv[3];

    bool samInput = stringEndsWith(argv[3], ".sam");
    ...
    options->fastqFile1 = samInput ? NULL : argv[4];

    for (int n = (samInput ? 4 : 5); n < argc; n++) {
...
void RNASeqAlignerContext::initialize()
{
    AlignerContext::initialize();
    RNASeqAlignerOptions* options2 = (RNASeqAlignerOptions*) options;
    ...
    if (index == NULL) {
        fprintf(stderr, "Transcriptome load failed, aborting.\n");
        exit(1);
    }
...
AlignerStats* RNASeqAlignerContext::newStats()
{
    return new RNASeqAlignerStats();
}

void RNASeqAlignerContext::runTask()
{
    ParallelTask<RNASeqAlignerContext> task(this);
    task.run();
}

void RNASeqAlignerContext::runIterationThread()
{
    int maxReadSize = 10000;
    RNASeqPairedEndAligner *aligner = new RNASeqPairedEndAligner(
            index,
            transcriptome,
            gtf,
            maxReadSize,
    ...
    while (fileSplitter->getNextRange(&rangeStart, &rangeLength)) {
        ...
        while (reader->getNextReadPair(&read0,&read1)) {
            ...
            updateStats((RNASeqAlignerStats*) stats, &read0, &read1, &result);
...
void RNASeqAlignerContext::writePair(Read* read0, Read* read1, PairedAlignmentResult* result)
{
    if (samWriter != NULL && (options->passFilter(read0, result->status[0]) || options->passFilter(read1, result->status[1]))) {
        samWriter->writePair(read0, read1, result);
    }
...
void RNASeqAlignerContext::updateStats(RNASeqAlignerStats* stats, Read* read0, Read* read1, PairedAlignmentResult* result)
{
    // Update stats
    for (int r = 0; r < 2; r++) {
