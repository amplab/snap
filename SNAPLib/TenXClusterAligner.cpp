...
TenXClusterALigner::TenXClusterALigner(
		...
		unsigned			maxClusterSize_,
	    unsigned            minReadLength_,
        int                 maxSecondaryAlignmentsPerContig,
        BigAllocator        *allocator)
		: underlyingPairedEndAligner(underlyingPairedEndAligner_), maxClusterSize(maxClusterSize_), forceSpacing(forceSpacing_), index(index_), minReadLength(minReadLength_)
{
    ...
    for (unsigned i = 0; i < maxClusterSize; i++)
		underlyingPairedEndAligner[i]->setLandauVishkin(&lv, &reverseLV);
