...
for input_format in ["fq", "fq.gz", "bam", "sam"]:
    for paired in [0, 1]:
        ...
        test_sorted = [0, 1] if input_format == "fq" or input_format == "bam" else [0]
        for output_format in test_output_format:
            for sorted in test_sorted:
                test_pipe = [0, 1] if sorted == 0 and (input_format == "bam" or input_format == "sam" or paired == 0) else [0]
                for pipe in test_pipe:
                    ...
                    for threads in test_threads:
                        ...
                        if sorted:
                            args.append("-so")
                        run = "%s-%s-%s-%s-%s-%s" % (input_format, ["single", "paired"][paired], output_format, ["unsorted", "sorted"][sorted], threads, ["file", "pipe"][pipe])
