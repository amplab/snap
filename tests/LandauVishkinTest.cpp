...
TEST_F(LandauVishkinTest, "CIGAR strings") {
    ...
    lvc.computeEditDistance("abcde", 5, "abcde", 5, 2, cigarBuf, bufLen);
    ASSERT_STREQ("5=", cigarBuf);

    lvc.computeEditDistance("abcdef", 6, "abcde", 5, 2, cigarBuf, bufLen);
    ASSERT_STREQ("5=", cigarBuf);

    lvc.computeEditDistance("abcde", 5, "abcdX", 5, 2, cigarBuf, bufLen);
    ASSERT_STREQ("4=1X", cigarBuf);   // This used to give 4=1I before

    lvc.computeEditDistance("abcde", 5, "Xbcde", 5, 2, cigarBuf, bufLen);
    ASSERT_STREQ("1X4=", cigarBuf);

    lvc.computeEditDistance("abcde", 5, "abde", 4, 2, cigarBuf, bufLen);
    ASSERT_STREQ("2=1D2=", cigarBuf);

    lvc.computeEditDistance("abcde", 5, "bcde", 4, 2, cigarBuf, bufLen);
    ASSERT_STREQ("1D4=", cigarBuf);

    lvc.computeEditDistance("abcde", 5, "abcXde", 6, 2, cigarBuf, bufLen);
    ASSERT_STREQ("3=1I2=", cigarBuf);

    lvc.computeEditDistance("abcde", 5, "abXXe", 5, 2, cigarBuf, bufLen);
    ASSERT_STREQ("2=2X1=", cigarBuf);

    lvc.computeEditDistance("abcde", 5, "abcXXde", 7, 3, cigarBuf, bufLen);
    ASSERT_STREQ("3=2I2=", cigarBuf);

    lvc.computeEditDistance("ttttc", 5, "tttc", 4, 3, cigarBuf, bufLen);
    ASSERT_STREQ("3=1X", cigarBuf);

    lvc.computeEditDistance("tttcc", 5, "ttttc", 5, 3, cigarBuf, bufLen);
    ASSERT_STREQ("3=1X1=", cigarBuf);

    lvc.computeEditDistance("tttcc", 5, "tttaa", 5, 3, cigarBuf, bufLen);
    ASSERT_STREQ("3=2X", cigarBuf);

    // A real example where we used to give 1D and later 1I instead of 2X
    lvc.computeEditDistance("atctcag", 7, "acttcag", 7, 3, cigarBuf, bufLen);
    ASSERT_STREQ("1=2X4=", cigarBuf);

    // Edge cases when pattern is longer than available text
    lvc.computeEditDistance("abc", 3, "abcde", 5, 3, cigarBuf, bufLen);
    ASSERT_STREQ("3=2X", cigarBuf);

    lvc.computeEditDistance("abc", 3, "abXde", 5, 3, cigarBuf, bufLen);
